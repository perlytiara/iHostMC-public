...
import { Search, Download, ExternalLink, Loader2, Plug } from "lucide-react";
import { useCallback, useState } from "react";
import { useTranslation } from "react-i18next";
import { cn } from "@/lib/utils";
import type { ModrinthHit, SpigetResource, CurseForgeHit } from "../types";

interface BrowsePluginsProps {
  serverId: string;
  gameVersion: string;
}
...
export function BrowsePlugins({ serverId, gameVersion }: BrowsePluginsProps) {
  const { t } = useTranslation();
  const [query, setQuery] = useState("");
  const [source, setSource] = useState<Source>("modrinth");
  const [modrinthResults, setModrinthResults] = useState<ModrinthHit[]>([]);
  const [spigetResults, setSpigetResults] = useState<SpigetResource[]>([]);
  ...
  const search = useCallback(async () => {
    ...
    try {
      if (source === "modrinth") {
        const hits = await invoke<ModrinthHit[]>("search_modrinth_plugins", {
          query: query.trim(),
        ...
        const hits = await invoke<SpigetResource[]>("search_spiget_plugins", {
          query: query.trim(),
          size: 20,
        });
        setSpigetResults(hits);
      } else {
        const hits = await invoke<CurseForgeHit[]>("search_curseforge_plugins", {
          query: query.trim(),
          gameVersion: gameVersion || undefined,
          limit: 20,
        });
  ...
  }, [query, gameVersion, source]);

  const installModrinth = useCallback(async (slug: string) => {
    setInstalling(slug);
    try {
      await invoke("install_modrinth_plugin", { serverId, projectSlug: slug, gameVersion });
    } catch (e) {
      console.error(e);
    } finally {
      setInstalling(null);
  ...
  const installSpiget = useCallback(async (id: number, premium: boolean) => {
    if (premium) {
  ...
  const installCurseforge = useCallback(async (modId: number) => {
    ...
    try {
      await invoke("install_curseforge_plugin", { serverId, modId, gameVersion });
    } catch (e) {
      console.error(e);
    } finally {
      setInstalling(null);
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      ...
      <div className="flex gap-1">
        {(["modrinth", "spigot", "curseforge"] as const).map((s) => (
          <button
            key={s}
            type="button"
            className={cn(
            ...
            onClick={() => setSource(s)}
          >
            {s === "modrinth" ? "Modrinth" : s === "spigot" ? "SpigotMC" : "CurseForge"}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
              ...
              {modrinthResults.map((m) => (
                <li key={m.slug} className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 transition-colors hover:bg-accent/30">
                  {m.icon_url ? (
                    ...
                    <div className="flex h-10 w-10 items-center justify-center rounded-md bg-muted"><Plug className="h-5 w-5 text-muted-foreground" /></div>
                  )}
        ...
        {source === "spigot" && (
          spigetResults.length === 0 && !loading ? (
            <EmptyState message={t("mods.searchHint")} />
          ) : (
            <ul className="space-y-2">
              {spigetResults.map((r) => {
                ...
                return (
                  <li key={r.id} className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 transition-colors hover:bg-accent/30">
                    <div className="flex h-10 w-10 items-center justify-center rounded-md bg-muted"><Plug className="h-5 w-5 text-muted-foreground" /></div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm text-foreground truncate">{r.name}</p>
                      {r.tag && <p className="text-xs text-muted-foreground line-clamp-1">{r.tag}</p>}
                    </div>
                    {premium ? (
                      ...
                      <Button size="sm" variant="outline" className="gap-1.5 shrink-0" disabled={installing === String(r.id)} onClick={() => installSpiget(r.id, premium)}>
                        {installing === String(r.id) ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Download className="h-3.5 w-3.5" />}
                        {t("mods.install")}
                      </Button>
                    )}
                  </li>
